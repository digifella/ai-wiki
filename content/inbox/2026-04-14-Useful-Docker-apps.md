...
---
---
Here is a detailed [[concepts/summary|summary]] of the video "5 Underrated [[concepts/docker-containers|Docker Containers]] for Your [[concepts/home-lab|Home Lab]]" formatted in [[concepts/markdown|Markdown]].
<https://youtu.be/hgaP-ndM9Ds>

# 5 Underrated Docker Containers for Your Home Lab

This video [[concepts/highlights|highlights]] five Docker containers that may not always get the spotlight but offer significant utility for home lab users. From [[concepts/document-management|document management]] to [[concepts/server-monitoring|server monitoring]], these tools prioritize [[concepts/privacy|privacy]], ease of use, and self-hosting [[concepts/capabilities|capabilities]].

* * *

## 1\. Stirling-PDF

**The Privacy-Focused [[concepts/pdf-manipulation|PDF Manipulation]] Tool**

Many users rely on expensive [[concepts/software|software]] like [[entities/adobe-acrobat|Adobe Acrobat]] for simple PDF tasks, or worse, use free online converters that require uploading sensitive documents to third-party servers. Stirling-PDF offers a robust, local alternative.

	**Primary Function:** A web-based local PDF manipulation tool.
	
	**Key Features:**
		**Editing:** Merge, split, reorder, rotate, and compress [[concepts/pdfs|PDFs]].
		
		**Conversion:** Convert [[concepts/files|files]] to and from PDF formats.
		
		**[[concepts/security|Security]]:** Sign documents, add watermarks, and extract pages.
...
While many home lab users monitor _uptime_ (is the server on?), fewer monitor actual system _health_ ([[concepts/cpu|CPU]], [[concepts/ram|RAM]], Disk usage). While tools like Grafana are powerful, they can be complex to set up. Beszel offers a simpler [[concepts/solution|solution]].
	...
	**How it Works:** You install the main container (Hub) and a lightweight [[entities/agent|agent]] on the [[concepts/systems|systems]] you want to monitor.
	
	**Key Features:**
		**Metrics:** Monitors CPU usage, [[concepts/memory|memory]] usage, disk space, and disk I/O.
	...
	**Comparison:** It complements tools like **[[concepts/uptime|Uptime]] Kuma**. Use Uptime Kuma to know if a service is down; use Beszel to know _why_ (e.g., the disk is full).
...
Logging into multiple different services with different credentials is tedious and insecure. Authentik acts as an Identity Provider (IdP) to centralize access.
	...
	**Key Features:**
		**Centralized Security:** [[concepts/secure|Secure]] one account with a strong password and 2FA, then use that account to access all other apps (Proxmox, Portainer, etc.).
		
		**[[concepts/integration|Integration]]:** Supports standard protocols like OAuth2 and OIDC to integrate with various self-hosted applications.
...
Nextcloud is often criticized for being "bloated" or difficult to maintain. However, the presenter argues it is underrated specifically because of the **Nextcloud AIO (All-In-One)** installer and its desktop sync features.
	...
	**Primary Function:** Self-hosted file [[entities/storage|storage]] and collaboration ([[entities/google|Google]] Drive/Synology Drive alternative).
	
	**The AIO Advantage:** The All-In-One installer automates the difficult parts of [[concepts/setup|setup]]. It handles containers, backups, and updates automatically, removing the maintenance headache.
