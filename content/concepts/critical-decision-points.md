---
type: concept
domain: philosophy-religion
tags:
  - "critical-decisions"
  - "ai"
  - "decision-making"
updated: 2026-04-14
group: philosophy-ethics-logic
---
# Critical Decision Points

Moments requiring evaluation of alternatives with high impact on outcomes, where misjudgment leads to significant consequences. Requires structured frameworks to navigate complexity.

## Examples

- **The [[concepts/cockpit-rule|Cockpit Rule]]**: Mental model for deciding when to delegate to AI, collaborate with AI, or avoid it entirely (from AI framework [[concepts/skills|skills]] - [[entities/jeff-su|Jeff Su]] Channel). Misapplication causes inefficiency in AI [[concepts/adoption|adoption]].

## Related Concepts

- AI
- Decision-Making Frameworks
- [[concepts/ai-proficiency|AI Literacy]]

Backlink: 2026 04 14 AI framework skills [[entities/jeff-su|Jeff Su]] Channel

## Source Notes
- 2026-04-08: Anthropic
- 2026-04-10: [[lab-notes/2026-04-10-Anthropics-Claude-AI-Subscription-Changes-OpenClaw-Ban-Usage-Limits-an|Anthropics Claude AI Subscription Changes OpenClaw Ban Usage Limits an]] · [▶ source](https://www.youtube.com/watch?v=a4hdPWSUzsE)
- 2026-04-24: Strategies to Transform Claude AI · [▶ source](https://www.youtube.com/watch?v=c68ha7pY9aE)
- 2026-04-27: Git