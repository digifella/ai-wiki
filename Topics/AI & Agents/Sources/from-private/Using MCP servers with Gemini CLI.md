...
This video demonstrates how to configure and use Model Context Protocol (MCP) servers, specifically Bright Data, with the [[concepts/gemini|Gemini]] [[concepts/cli|Command Line]] Interface (CLI) and [[entities/claude-cowork|Claude Desktop]] for enhanced web [[concepts/data-extraction|data extraction]].
**Key Steps and Concepts:**

1. **Initial Check (0:08):** The video starts by showing that the [[entities/gemini-cli|Gemini CLI]] initially has "No MCP servers configured" when running the `/mcp` command.
...
3. **Introducing Bright Data MCP (1:18):**
	The video highlights Bright Data MCP, a service that enables Large Language Models (LLMs) and agents to access, discover, and extract real-time web data, particularly from websites that are usually hard to scrape (e.g., LinkedIn, [[entities/amazon|Amazon]], [[entities/youtube|YouTube]]). Bright Data is a paid service but offers credits for users to try.
...
5. **Obtaining API Key and Zone from Bright Data (2:35):**
	Users need to sign up/log in to Bright Data. Navigate to "Proxies & Scraping" and create a new "Web Unlocker API" zone. The video recommends enabling "CAPTCHA Solver". After creation, the API Key and Zone Name (e.g., `web_unlocker4`) are provided and should be copied into the `settings.json` file.
	
6. **Verifying Configuration in Gemini CLI (4:07):**
	After saving `settings.json`, restarting the Gemini CLI (`gemini`) is necessary for changes to take effect. Running `/mcp` again now shows "Bright Data - Ready (48 tools)", indicating successful configuration and listing all available web scraping tools (e.g., `web_data_youtube_profiles`, `web_data_linkedin_job_listings`, `web_data_amazon_product`).
	
7. **Demonstrating Web Scraping with Gemini CLI (4:50):**
	A query is sent to Gemini CLI: "what are the latest stats of the youtube channel https://www.youtube.com/@engineerprompt . what are the last 10 videos on this channel". Gemini CLI identifies and asks permission to use the `web_data_youtube_profiles` tool from "Bright Data". Upon approval, it successfully extracts and presents YouTube channel statistics (subscribers, total views, videos, creation date) and the last 10 videos with their links and views. The video also shows how to check for errors (Ctrl+O) which revealed a "Too Many Requests" error for the free tier, causing a fallback to [[entities/gemini-25-flash|Gemini 2.5 Flash]] model.
...
- [[concepts/bright-data|Bright Data]] — [Wikipedia](https://en.wikipedia.org/wiki/Bright_Data)
- [[concepts/settingsjson-file|settings.json file]] — [Wikipedia](https://en.wikipedia.org/wiki/settings.json_file)
...
- Web unlocker zones — [Wikipedia](https://en.wikipedia.org/wiki/Web_unlocker_zones)
- Rate limiting — [Wikipedia](https://en.wikipedia.org/wiki/Rate_limiting)
- CAPTCHA solvers — [Wikipedia](https://en.wikipedia.org/wiki/CAPTCHA_solvers)
- Tool inputs and limitations — [Wikipedia](https://en.wikipedia.org/wiki/Tool_inputs_and_limitations)

## Related Entities
- [[entities/gemini|Gemini]] — [Wikipedia](https://en.wikipedia.org/wiki/Gemini)
- [[entities/bright-data|Bright Data]] — [Wikipedia](https://en.wikipedia.org/wiki/Bright_Data)
- [[entities/claude-desktop|Claude Desktop]] — [Wikipedia](https://en.wikipedia.org/wiki/Claude_Desktop)