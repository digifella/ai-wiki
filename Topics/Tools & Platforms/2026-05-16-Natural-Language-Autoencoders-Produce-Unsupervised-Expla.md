...
We apply NLAs to model auditing. During our pre-[[concepts/deployment|deployment]] audit of [[entities/claude-opus-46|Claude Opus 4.6]], NLAs helped diagnose safety-relevant behaviors and surfaced unverbalized evaluation awareness—cases where Claude believed, but did not say, that it was being evaluated. We present these audit findings as case studies and corroborate them using independent methods. On an automated auditing benchmark requiring end-to-end investigation of an intentionally-misaligned model, NLA-equipped agents outperform baselines and can succeed even without access to the misaligned model’s [[concepts/language-data|training data]].
