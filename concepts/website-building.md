---
type: concept
domain: tools-platforms-infrastructure
tags:
  - "website-building"
  - "no-code"
  - "ai-development"
  - "claude-code"
  - "nano-banana"
  - "web-publishing"
  - "automated-workflows"
aliases:
  - "Web Development"
  - "Web Site Creation"
summary: The discipline of designing, constructing, and publishing web pages via the internet using traditional coding, no-code platforms, and AI-driven development workflows.
updated: 2026-07-09
group: web-publishing-quartz-websites
generated: { by: "process:nemoclaw-wiki-ingest", at: "2026-07-09" }
---
<!-- domain-nav -->
> domain-badge slug=tools-platforms-infrastructure name=Tools, Platforms & Infrastructure

# Website Building

[[concepts/slms|Definition]]: The discipline of designing, constructing, and publishing web pages or sites for public viewing via the internet.

## Methodologies
- [[concepts/no-code|No-Code]] platforms and builders
- AI-driven automated [[concepts/development-workflows|development workflows]]
- Traditional hand-[[concepts/coding|coding]] frameworks

## AI Integration & Rapid Deployment
Recent advancements enable rapid creation of high-fidelity sites without traditional [[concepts/coding|coding]]:
- **Workflow:** [[concepts/claude-code|Claude Code]] & [[concepts/nano-banana-2|Nano Banana 2]] integration
- **Capabilities:** Generates aesthetically pleasing, responsive, and SEO-optimized websites
- **[[concepts/accessibility|Accessibility]]:** Designed for individuals with no prior coding [[concepts/experience|experience]]
- **[[concepts/economic-impact|Economic Impact]]:** Demonstrated potential for generating high-value assets (e.g., $10,000 websites)

## Case Studies & Resources
- **Presenter:** [[entities/jack-roberts|Jack Roberts]]
- **Topic:** [[entities/claude-code|Claude Code]] + [[entities/nano-banana-2|Nano Banana 2]] = Insane $10,000 Websites
- **Media:** [YouTube Video](https://www.youtube.com/watch?v=TZUTe7s11-I)

## Backlinks
- 2026 04 14 [[concepts/ai-assisted-coding|Claude Code]] and [[concepts/camera-angle-control|nano Banana 2]]
## Source Notes
- 2026-04-14: "But [[concepts/openclaw|OpenClaw is expensive..."]]
- 2026-04-10: Full Guide - Build Your Own AI Second Brain with [[entities/claude-code|Claude Code]]
- 2026-04-07: [[lab-notes/2026-04-07-Claude-Code-AI-Marketing-Suite-Democratizing-Comprehensive-Audits-and|Claude Code AI Marketing Suite Democratizing Comprehensive Audits and]] · [▶ source](https://www.youtube.com/watch?v=eorc3jLBqIA)
- 2026-04-24: Hermes · [▶ source](https://www.youtube.com/watch?v=4Sln_6K2z8c)
- 2026-05-01: [[lab-notes/2026-05-01-Modern-AI-Agentic-Harness-Architecture-Components-and-Fr|Modern AI Agentic Harness: Architecture, Components, and Framework Differences]] · [▶ source](https://www.youtube.com/watch?v=nWzXyjXCoCE)
