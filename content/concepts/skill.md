...
[[concepts/agent-harnesses|Agent Skills]] are a technical feature in [[concepts/claude-ai|Claude]] that enable [[concepts/agentic-ai|AI agents]] to perform specific, well-defined tasks by leveraging structured function definitions. [[concepts/skills|Skills]] function as a bridge between an [[entities/agent|agent]]'s [[concepts/reasoning-capabilities|reasoning capabilities]] and external systems or computational operations, allowing [[concepts/claude|Claude]] to understand what actions are available and when to invoke them appropriately.

## Implementation and Function

Skills are implemented through a formal interface that provides Claude with [[concepts/metadata|metadata]] about available operations, including their [[concepts/motivation|purpose]], required [[concepts/parameters|parameters]], and expected outputs. When an agent encounters a problem requiring external action, it can identify and call the appropriate skill with the correct inputs. This [[concepts/architecture|architecture]] allows [[concepts/agents|agents]] to extend their [[concepts/capabilities|capabilities]] beyond language-based [[concepts/reasoning|reasoning]] into concrete task execution, such as data retrieval, calculations, or system interactions.

## Design Principles

The effectiveness of [[concepts/agent-skills|Agent Skills]] depends on clear definition and appropriate granularity. Each skill should represent a single, meaningful capability with unambiguous inputs and outputs. This specificity helps Claude's [[concepts/decision-making|decision-making]] process by making it easier for the model to recognize when and how to apply each skill, reducing errors and improving the [[concepts/software-reliability|reliability]] of agent behavior in [[concepts/complex-workflows|complex workflows]].
## Source Notes
