---
wiki-ingested: true
title: "Glioblastoma Reprogramming: Transforming Brain Cancer Cells into Functional Neurons"
date: 2026-06-14
source_type: youtube_summary
provider: Google
api: Gemini 2.5 Flash
modes: Summary
domain: biology-life-sciences
group: life-systems-adaptation-discovery
type: "source-summary"
---
<!-- domain-nav -->
> domain-badge slug=biology-life-sciences name=Biology & Life Sciences

Generated: 2026-06-14 · API: [[concepts/gemini-25-models|Gemini 2.5]] Flash · Modes: Summary

---

## Glioblastoma Reprogramming: Transforming Brain Cancer Cells into Functional Neurons
**Clip title:** How to Turn Cancer Into Brain Cells
**Author / channel:** SciShow
**URL:** https://www.youtube.com/watch?v=HoYY9J2w75E

### Summary
This video from [[entities/scishow-channel|SciShow]] details groundbreaking research that aims to transform aggressive brain cancer cells into functional brain cells, offering a novel approach to treating one of the deadliest cancers. The main topic revolves around the challenges of treating glioblastoma and a promising new method involving radiation and a chemical compound that can reprogram these cancerous cells.

Glioblastoma is an exceptionally aggressive brain cancer with a median survival time of only 15 to 18 months after diagnosis, making it particularly difficult to treat compared to other cancers. Its location within the skull presents significant surgical challenges, and the blood-brain barrier restricts the effectiveness of chemotherapy. While radiotherapy can damage cancer cells' DNA, a combination of these traditional treatments is often insufficient as some cancer cells typically survive and continue to spread. The core problem is finding a way to eliminate or neutralize these persistent cancer cells without causing irreparable damage to vital brain functions.

Researchers at UCLA embarked on an ambitious project to revert glioblastoma cells back to a non-lethal state, leveraging the understanding that both neurons and glial cells (which can become cancerous glioblastomas) originate from neural stem cells through differentiation, a process influenced by molecules like cyclic AMP (cAMP). Initial *in vitro* experiments demonstrated that treating glioblastoma cells with radiation (which made them more stem-cell-like) followed by a cAMP analog successfully induced them to adopt neuron-like shapes, produce neuronal proteins, and stop dividing. Recognizing the limitations of the cAMP analog in a living organism (short half-life, inability to cross the blood-brain barrier), the team instead used Forskolin, a molecule known to boost natural cAMP production and penetrate the blood-brain barrier effectively.

This combined radiation and Forskolin treatment showed remarkable results in mice, increasing their average survival from 36 days to 129 days—more than triple the original lifespan—by reducing glioblastoma cell counts and nudging remaining cells back to a pre-cancerous state. While testing human glioblastoma cells implanted in mice yielded a smaller survival increase (from 34 to 48 days), the research nonetheless demonstrates a significant step towards reprogramming cancer. This innovative approach could revolutionize glioblastoma treatment by turning deadly cells into beneficial ones, although further optimization and research are needed before it can be applied in human patients.

### Video Description & Links
#### Description
Visit https://brilliant.org/scishow/ to get started [[concepts/learning|learning]] STEM for free for a full 30 days and get 20% off your annual premium subscription.

Leave a comment about the ISS below (iykyk)! 

Brain cancer, glioblastoma, is super deadly. But what if we could turn those deadly cancer cells into useful brain cells?

Hosted by: Savannah Geary (they/them)
----------
Support us for $8/month on Patreon and keep SciShow going!
https://www.patreon.com/scishow
Or support us directly: https://complexly.com/support
Join our SciShow [[entities/email|email]] list to get the latest news and highlights: 
https://mailchi.mp/scishow/email
----------
Huge thanks go to the following Patreon supporters for helping us keep SciShow free for everyone forever: Shaji [[entities/chef-john|John]], Timos Gies, Jon Coffman, Anita, Anne Herrington, Ashley Moquin, yeyette, [[entities/david|David]] Johnston, Cye Stoner, Jp Lynch, Bethany Matthews, Chris Curry, J.V. Rosenbalm, Blood Doctor Kelly, Toyas Dhake, Reed Spilmann, Garrett Galloway, Friso, Lyndsay Brown, [[entities/jeremy|Jeremy]] Mattern, Jaap Westera, Matt Curls, Eric [[entities/jensen-huang|Jensen]], Chris Mackey, Adam Brainard, Piya Shedden, Steve Gums, Alex [[entities/hackman|Hackman]], Kevin Knupp, Chris Peters, Kevin Bealer, Joseph Ruf, Jason A Saslow
----------
Looking for SciShow elsewhere on the internet?
SciShow Tangents Podcast: https://scishow-tangents.simplecast.com/
[[entities/tiktok|TikTok]]: https://www.tiktok.com/@scishow 
Instagram: http://instagram.com/thescishow
Facebook: http://www.facebook.com/scishow
Bluesky: https://bsky.app/profile/scishow.bsky.social

#SciShow #science #education #learning #complexly
----------
Sources:
https://docs.google.com/document/u/1/d/e/2PACX-1vS2YE0Qvi59ExsnbbgFyWgHx2tG0XKTISKyohxi558vXYDAw6oeUuZ_xRwSqDK2WiQLRqOl_V5rEiYk/pub

#### Tags
`SciShow`, `science`, `Hank`, `Green`, `education`, `learn`, `complexly`, `hank green`, `Making Brain Cells From Recycled Cancer`, `brain cells`, `cancer`, `cancer research`, `brain cancer`, `brain cancer research`, `cancer cells`, `understanding cancer`

#### URLs
- https://brilliant.org/scishow/
- https://www.patreon.com/scishow
- https://complexly.com/support
- https://mailchi.mp/scishow/email
- https://scishow-tangents.simplecast.com/
- https://www.tiktok.com/@scishow
- http://instagram.com/thescishow
- http://www.facebook.com/scishow
- https://bsky.app/profile/scishow.bsky.social
- https://docs.google.com/document/u/1/d/e/2PACX-1vS2YE0Qvi59ExsnbbgFyWgHx2tG0XKTISKyohxi558vXYDAw6oeUuZ_xRwSqDK2WiQLRqOl_V5rEiYk/pub

## Related Concepts
- [[concepts/glioblastoma-reprogramming|Glioblastoma Reprogramming]] — [Wikipedia](https://en.wikipedia.org/wiki/Glioblastoma_Reprogramming)
- [[concepts/radiation-therapy|Radiation Therapy]] — [Wikipedia](https://en.wikipedia.org/wiki/Radiation_Therapy)
- [[concepts/chemical-compound-reprogramming|Chemical Compound Reprogramming]] — [Wikipedia](https://en.wikipedia.org/wiki/Chemical_Compound_Reprogramming)
- [[concepts/functional-neurons|Functional Neurons]] — [Wikipedia](https://en.wikipedia.org/wiki/Functional_Neurons)
- [[concepts/radiation-therapy|Cancer Treatment]] — [Wikipedia](https://en.wikipedia.org/wiki/Cancer_Treatment)
- [[concepts/radiation-therapy|Glioblastoma]] — [Wikipedia](https://en.wikipedia.org/wiki/Glioblastoma)
- [[concepts/radiation-therapy|Cell Reprogramming]] — [Wikipedia](https://en.wikipedia.org/wiki/Cell_Reprogramming)
- Blood-Brain Barrier — [Wikipedia](https://en.wikipedia.org/wiki/Blood-Brain_Barrier)
- Cyclic AMP (cAMP) — [Wikipedia](https://en.wikipedia.org/wiki/Cyclic_AMP_%28cAMP%29)
- Forskolin — [Wikipedia](https://en.wikipedia.org/wiki/Forskolin)
- Neural Stem Cells — [Wikipedia](https://en.wikipedia.org/wiki/Neural_Stem_Cells)
- Cell Differentiation — [Wikipedia](https://en.wikipedia.org/wiki/Cell_Differentiation)
- Challenges in Brain Cancer Treatment — [Wikipedia](https://en.wikipedia.org/wiki/Challenges_in_Brain_Cancer_Treatment)
- In Vitro Experiments — [Wikipedia](https://en.wikipedia.org/wiki/In_Vitro_Experiments)
- Pre-clinical Trials — [Wikipedia](https://en.wikipedia.org/wiki/Pre-clinical_Trials)

## Related Entities
- [[entities/scishow-channel|SciShow]] — [Wikipedia](https://en.wikipedia.org/wiki/SciShow)
- [[entities/gemini-25-flash|Gemini 2.5 Flash]] — [Wikipedia](https://en.wikipedia.org/wiki/Gemini_2.5_Flash)
- UCLA — [Wikipedia](https://en.wikipedia.org/wiki/UCLA)
- Savannah Geary — [Wikipedia](https://en.wikipedia.org/wiki/Savannah_Geary)
- Brilliant — [Wikipedia](https://en.wikipedia.org/wiki/Brilliant)
- Patreon — [Wikipedia](https://en.wikipedia.org/wiki/Patreon)
- Complexly — [Wikipedia](https://en.wikipedia.org/wiki/Complexly)