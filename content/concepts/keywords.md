...
Within Lightroom's catalog system, keywords are applied through the Keywording panel and can be managed via a keyword list that supports nested hierarchies. This allows for both broad categorization (such as "landscape") and specific tagging (such as "landscape > mountain > alpine"). Keywords become embedded in image metadata, making them searchable across the entire catalog and portable to other [[concepts/software|applications]] or systems that read standard metadata formats.
