...
Here is a detailed summary of the video [[concepts/tutorial|tutorial]] on setting up and running local Reinforcement [[concepts/learning|Learning]] (RL) using Nvidia and [[entities/unsloth|Unsloth]].
...
* **Specific Technique:** The tutorial uses **Reinforcement Learning with Verifiable Rewards**.
	* _How it works:_ The AI is placed in an environment where it attempts tasks. It is automatically given points (rewards) for success or penalties for failure. Humans are removed from the training [[concepts/loop|loop]], allowing the AI to iterate and learn rapidly.
