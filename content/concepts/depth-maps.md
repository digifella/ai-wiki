...
Depth maps are grayscale [[concepts/images|images]] that store spatial depth information for a photograph or 3D scene, where pixel brightness values represent distance from the camera. In [[concepts/photoshop|Photoshop]], depth maps serve as precise masks that enable selective [[concepts/adjustments|adjustments]] based on spatial position rather than color or [[concepts/contrast|contrast]] alone. This technique allows photographers and designers to apply effects like blur, sharpening, color correction, or lighting changes to specific depth planes within an image.

## Creating and Using Depth Maps

Depth maps can be generated through several methods: extracted from cameras with built-in depth sensors, created from stereo image pairs, or produced through computational photography algorithms. Once imported into Photoshop, these maps function as selection or adjustment masks, enabling adjustments that respect the three-dimensional [[concepts/structure|structure]] of the scene. Common [[concepts/software|applications]] include selectively blurring backgrounds while keeping foregrounds sharp, adjusting focus planes retroactively, or creating atmospheric depth effects.
