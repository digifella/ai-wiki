---
wiki-ingested: true
title: "Build an agentic rag system in azure ai and foundry"
...
[[concepts/azure-innovation-station|Azure Innovation Station]] channel

Here is a [[concepts/markdown|markdown]] guide based on the video [[concepts/tutorial|tutorial]] for creating an [[concepts/ai-agent|AI Agent]] using Agentic RAG (Retrieval-Augmented Generation) in [[entities/microsoft|Microsoft]] Azure.

* * *

# How to Create an Agentic RAG AI Agent in Azure

This guide outlines the process of setting up an AI agent that uses Retrieval-Augmented Generation (RAG) to answer questions based on specific documents ([[concepts/e-books|e-books]]) rather than general knowledge.
...
1. Navigate to the **[[entities/azure-portal|Azure Portal]]**.
2. Create a new [[concepts/resource-group|resource group]] (or use an existing empty one).
3. Search for and create a **[[entities/microsoft-foundry|Microsoft Foundry]]** resource.
	* _Note: Previously known as [[entities/azure-ai|Azure AI]] Foundry._
4. **Configuration:**
...
4. **Agent Details:**
	* **Name:** `e-book-agent`.
	* **Model:** The system defaults to **[[entities/gpt-4|GPT-4]].1** (You can deploy other [[concepts/models|models]] via the "Models" tab if desired).
5. **System [[concepts/prompting|Prompting]]:**
	* In the [[concepts/instructions|instructions]] pane, ask the AI to generate a restrictive prompt:
		"Create a [[concepts/system-prompt|system prompt]] that [[entities/will|will]] encourage our AI agent to only pull from an [[concepts/ai-search|AI search]] [[concepts/knowledge-base|knowledge base]] with the ebook data that we upload."
...
5. **Enrichment:**
	* Check **Enable semantic ranker** (improves [[concepts/search-relevance|search relevance]]).
