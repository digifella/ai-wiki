---
type: entity
summary: Gemma 3 is a model architecture developed by Google that includes multimodal capabilities and specialized variants such as MedGemma 27B.
updated: 2026-05-23
stub: true
---
# Gemma 3

[[entities/google-gemma|Gemma]] 3 is a model [[concepts/architecture|architecture]] developed by [[entities/google]].

## Specialized Variants
- [[concepts/dermatology-images|MedGemma 27B]]: A medical AI model built on the [[concepts/gemma-3-architecture|Gemma 3 architecture]], specifically trained for medical [[concepts/text|text]] and image comprehension.
	- **4B Multimodal**: Available in both pre-trained and instruction-tuned versions.
	- **27B Variant**: High-parameter version for complex medical [[concepts/reasoning|reasoning]].

## Backlinks
- [[concepts/date-2026-04-13|2026]] 04 14 [[entities/medgemma-27b|MedGemma 27B]] [[entities/fahd-merza|Fahd Merza]]
## Source Notes

- 2026-04-07: [[lab-notes/2026-04-07-Benchmarking-SLMs-Identifying-4GB-General-Problem-Solving-Champions|Benchmarking SLMs Identifying 4GB General Problem Solving Champions]] · [▶ source](https://www.youtube.com/watch?v=wQxawC3sv68)
- 2026-04-10: [[lab-notes/2026-04-10-LM-Studio-LM-Link-Remote-LLM-Access-for-Portable-Devices|LM Studio LM Link Remote LLM Access for Portable Devices]] · [▶ source](https://www.youtube.com/watch?v=PqBrnip-ZLw)