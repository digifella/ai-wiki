...
**Token-Based Text Splitting** Recognizing that Language [[concepts/models|Models]] (LLMs) process text in "tokens" rather than individual characters, token-based splitting aims to align chunking with how LLMs understand language. \[6:37, 6:49\] Tokens can be full words, parts of words, or punctuation, and language models use tokenizers (like [[entities/openai|OpenAI]]'s cl100k\_base tokenizer) to convert text into numerical representations. \[7:05, 7:47\] Token-based chunking can be more efficient for LLMs. \[6:55\] A 400-token chunk size with no overlap yielded 440 chunks for "Pride and Prejudice." \[9:14, 9:26\] Overlapping 400-token chunks with a 200-token overlap resulted in 878 chunks. \[10:00\]
