...
---
# [[concepts/private-rag-system|Private RAG system]] using [[entities/notebook-lm|notebookLM]]
...
* **Ollama ([[concepts/local-llm|Local LLM]] [[concepts/inference|Inference]])**: Used for generating text responses and embeddings from local [[concepts/large-language-models|large language models]] (LLMs). The demo uses `qwen3:8b-q4_K_M` and `nomic-embed-text`.
...
4. **Configure Environment Variables:** Make a copy of `.env.example` (in `local-ai-packaged`) and rename it to `.env`. Update required secret keys for n8n, Supabase, and optionally Neo4j/Langfuse. Supabase API keys (JWT Secret, Anon, Service Role) need to be auto-generated from the local Supabase dashboard. Copy contents from `insights-lm-local-package/.env.copy` into the main `.env` file for specific InsightsLM webhook URLs and Whisper/Coqui-TTS settings.
