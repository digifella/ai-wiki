...
# Using Ralph loops inside [[concepts/claude-code|Claude Code]]
...
The Ralph Wiggum plugin is a new tool designed to combat "[[concepts/ai-laziness|AI Laziness]]" in [[concepts/agentic-coding-tools|agentic coding tools]] like Claude Code. Named after the persistent _Simpsons_ character, the plugin changes Claude Code from a tool that tries once and gives up, into a loop that persistently iterates until a specific goal is objectively met.
...
The plugin forces a **[[concepts/persistent-iteration|persistent iteration]]** [[concepts/philosophy|philosophy]]. Instead of hoping the AI behaves, the plugin uses code to force the AI to stay in the [[concepts/session|session]] until the job is actually done.

### **How It Works: The Stop Hook**

The plugin utilizes a Claude Code feature called **Hooks** (user-defined shell [[concepts/commands|commands]] executed at specific lifecycle points). Specifically, it uses a **Stop Hook**:
...
### **1\. The Model: [[entities/claude-opus-4|Claude Opus 4]].5**

The video strongly recommends using **Claude Opus 4.5** with this plugin.

* **Why:** Lower-tier [[concepts/models|models]] (Haiku/Flash) lack the [[concepts/reasoning-capabilities|reasoning capabilities]] to unstuck themselves. They will likely enter an infinite loop of making the same mistake repeatedly. Opus 4.5 has the reasoning to analyze _why_ it failed and try a new approach.
