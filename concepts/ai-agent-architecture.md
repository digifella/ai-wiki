...
[[concepts/domain-memory|Domain memory]]—the agent's ability to retain and leverage task-specific knowledge—is central to agent [[concepts/software-reliability|reliability]] and effectiveness. This includes both episodic [[concepts/memory|memory]] of past interactions and semantic memory of domain-specific [[concepts/factual-knowledge|facts]] and rules. Effective architectures integrate [[concepts/persistent-state|domain memory]] as a first-class component rather than an afterthought, enabling agents to learn from [[concepts/experience|experience]], avoid repeated mistakes, and apply [[concepts/relevant-knowledge|contextual knowledge]] to novel situations. The architecture must specify how memory is stored, retrieved, updated, and integrated into the [[concepts/reasoning-steps|reasoning process]].
