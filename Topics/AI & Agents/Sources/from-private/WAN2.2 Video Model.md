...
The video provides a detailed guide on installing and using the freshly released [[entities/wan|Wan]] 2.2 [[concepts/text-to-video|text-to-video]] and [[concepts/image-to-video|image-to-video]] models locally with [[entities/comfyui|ComfyUI]]. The [[entities/speaker|speaker]], Fahad Mirza, also explains the [[concepts/architectural-improvements|architectural improvements]] of Wan 2.2 and demonstrates its capabilities with several prompts.
Here's a summary of the video:

* **Introduction to Wan 2.2 (0:00-0:06, 0:21-0:37)**: The video highlights the high-quality video generation capabilities of the newly released Wan 2.2 model. It [[concepts/notes|notes]] that Wan 2.2 is initially available with three different models: a 5-billion parameter model for both text-to-video and image-to-video, and two 14-billion parameter models specifically for text-to-video and video-to-video.
...
* **Architectural Innovations of Wan 2.2 (9:00-10:15)**: Wan 2.2 builds on its predecessor with key innovations: **[[entities/mixture-of-experts|Mixture-of-Experts]] (MoE) [[concepts/architecture|Architecture]]**: It employs two specialized agents: one for high-noise early denoising stages (global [[concepts/structure|structure]] management) and another for low-noise later stages (fine detail refinement). **Parameter Efficiency**: Despite a total [[concepts/parameter-count|parameter count]] of 27 billion, it maintains [[concepts/inference|inference]] efficiency by activating only 14 billion parameters per step. **Expanded Training Dataset**: The model uses a substantially expanded training dataset, with details available on its [[entities/hugging-face|Hugging Face]] model card.
* **Sponsors (10:16-10:49, 11:57-12:10)**: The video acknowledges its sponsors, Eigent (a multi-[[concepts/agent-workflow|agent workflow]] [[concepts/desktop-application|desktop application]]) and [[entities/massed-compute|Massed Compute]] (a [[concepts/cloud-computing|cloud computing]] infrastructure provider).
...
## Related Concepts
- [[concepts/text-to-video-model|Text-to-Video Model]] — [Wikipedia](https://en.wikipedia.org/wiki/Text-to-Video_Model)
- [[concepts/video-generation|Video Generation]] — [Wikipedia](https://en.wikipedia.org/wiki/Video_Generation)
- [[concepts/image-to-video-model|Image-to-Video Model]] — [Wikipedia](https://en.wikipedia.org/wiki/Image-to-Video_Model)
...
- {'term': '[[concepts/text-to-video-model|Text-to-Video Model]]'} — [Wikipedia](https://en.wikipedia.org/wiki/%7B%27term%27%3A_%27Text-to-Video_Model%27%7D)

## Related Entities
- [[entities/fahad-mirza|Fahad Mirza]] — [Wikipedia](https://en.wikipedia.org/wiki/Fahad_Mirza)
- [[entities/wan-22|Wan 2.2]] — [Wikipedia](https://en.wikipedia.org/wiki/Wan_2.2)