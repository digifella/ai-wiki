---
type: concept
domain: entertainment-games
summary: This page covers the concept of training, including sports science applications and AI model training techniques.
updated: 2026-05-23
group: sports-science-training-recovery
---
- "sports-[[concepts/science|science]]"
  - "training-recovery"
  - "sports-games"
  - "ai-[[concepts/models|models]]"
  - "[[concepts/ai-image-generation|ai-image-generation]]"
aliases:
  - "training"
group: sports-science-training-recovery
stub: false
title: "training"

# training

*This page covers the concept of training, including sports science [[concepts/software|applications]] and AI model training techniques.*

## Sports Science Training
- Core concept in sports-science domain
- Related to training-recovery and sports-games

## AI Model Training
- Training [[concepts/ai-models]] like [[entities/flux1]] (from [[entities/black-forest-labs]])
- Techniques demonstrated in [[entities/adam-lucek]]'s video on training [[concepts/lora-adapter]] for image generation
- Low [[concepts/vram|VRAM]] image generation methods
- Tools and [[concepts/setup]] process for efficient training

## Related Content
- 2026 04 14 [[concepts/community-interest|Adam Lucek Flux model]] for Open AI generated image gen
## Source Notes
- 2026-04-07: [[lab-notes/2026-04-07-1-Bit-LLMs-BitNet-Bonsai-and-Efficient-On-Device-Deployment|1 Bit LLMs BitNet Bonsai and Efficient On Device Deployment]] · [▶ source](https://www.youtube.com/watch?v=0fWFetwHkVE)
- 2026-04-09: Photoshop
- 2026-04-10: [[lab-notes/2026-04-10-Meta-Muse-Spark-Features-Performance-and-Strategic-Shift-to-Proprietar|Meta Muse Spark Features Performance and Strategic Shift to Proprietar]] · [▶ source](https://www.youtube.com/watch?v=7vkybiVRSm0)
- 2026-04-12: [[lab-notes/2026-04-12-DreamDojo-AI-Bridging-Robotics-Sim2Real-Gap-for-Complex-Tasks|DreamDojo AI Bridging Robotics Sim2Real Gap for Complex Tasks]] · [▶ source](https://www.youtube.com/watch?v=mFSFvKquXwI)