...
Here is a detailed [[concepts/summary|summary]] of the video about Google's new [[concepts/gemini-cli|Gemini CLI]]:
...
4. **Authentication:** On first run (or by using the `/auth` command), you are given three options: **Login with Google:** The easiest method. It grants the 1,000 free daily requests. The speaker [[concepts/notes|notes]] that with this option, Google may use your data for [[concepts/training|training]], and you might be downgraded to the less powerful `gemini-2.5-flash` model during peak usage. **[[entities/gemini-api|Gemini API]] Key:** A paid option where you provide your own API key from [[entities/google-ai-studio|Google AI Studio]]. This ensures your data remains private and you consistently use the best model (Gemini 2.5 Pro). **Vertex AI:** A third option for enterprise users.
...
3. Add the following content to the `settings.json` file: `{"contextFileName": "AGENTS.md"}`. This configuration forces Gemini CLI to use the `AGENTS.md` file as its system prompt, allowing for consistent and highly-tuned instructions across different AI [[concepts/agents|agents]] without having to maintain separate prompt files.
