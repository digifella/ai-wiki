...
* (4:20) A significant cost comparison is provided: filtering 15 trillion tokens cost $60,000 using a fine-tuned BERT-based model, whereas doing the same with a low-cost decoder-only model like [[entities/gemini-3-flash|Gemini Flash]] would exceed $1 million, despite the lower per-token cost. This highlights the practical efficiency advantage of encoder-only models for certain workloads.
