...
An agentic tool is a capability or feature that enables an [[concepts/ai-agent|AI agent]] to perform tasks beyond simple [[concepts/text-generation|text generation]]. These tools extend an agent's functional scope by providing interfaces to external systems, computational environments, and information sources. Rather than generating text responses alone, agents equipped with tools can execute code, retrieve data, modify files, and interact with third-party services.

## Common Types of Tools

Agentic tools typically fall into several categories based on their function. [[concepts/code-execution|Code execution]] tools allow agents to run programming languages like [[concepts/python|Python]] or [[concepts/javascript|JavaScript]] within [[concepts/isolated-environments|sandboxed environments]]. [[concepts/research-tools|Research tools]] enable agents to search the web, query databases, or access document repositories. Integration tools connect agents to external services and [[concepts/open-standard-protocols|APIs]], allowing them to perform actions like sending messages, creating calendar events, or [[concepts/retrieving|retrieving]] data from [[concepts/business-applications|business applications]]. File manipulation tools permit agents to read, write, and modify documents and other stored content.

## Implementation Examples

Practical implementations of agentic tools appear in systems like [[concepts/claude-agent|Claude Code Agent]], which combines language understanding with the ability to write and execute code for solving [[concepts/computational-problems|computational problems]]. [[concepts/ai-integrated-notebooks|NotebookLM]] demonstrates [[concepts/acting|tool use]] through capabilities that allow agents to retrieve and synthesize information from documents. These implementations show how [[concepts/planning-errors|tool integration]] enables agents to work more autonomously and produce concrete outputs rather than just suggestions or explanations.
