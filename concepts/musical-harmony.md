---
type: concept
domain: creative-pursuits
tags:
  - "music"
  - "music-theory"
  - "harmony"
  - "harmonic-progression"
  - "musical-structure"
  - "chords"
  - "melody"
aliases:
  - "harmony"
summary: Musical harmony is the simultaneous sounding of different notes to create chords and provide vertical structure to a melody.
updated: 2026-07-11
group: photography-cameras
generated: { by: "process:nemoclaw-wiki-ingest", at: "2026-07-11" }
---
<!-- domain-nav -->
> domain-badge slug=creative-pursuits name=Creative Pursuits

# Musical harmony

The simultaneous sounding of different [[concepts/notes|notes]] to create Chords and provide vertical structure to a [[concepts/melody|Melody]].

### Core Principles
- Chords serve as the foundational building blocks of harmonic structure.
- Harmonic progression provides emotional depth and character to a [[concepts/melody|Melody]].
- Harmony is fundamentally shaped by the structural [[concepts/relationships|relationships]] between Chords and [[concepts/musical-scales|Scales]].

### Related Concepts
- Music [[concepts/theory|theory]]
- [[concepts/counterpoint|Counterpoint]]
- [[concepts/tonal-music|Tonal music]]
- [[concepts/dissonance|Dissonance]]

---
**Source:** 2026 04 27 [[concepts/music-chords|Music Chords]] Foundations Anatomy Harmony and Scale Relat
## Source Notes
- 2026-04-27: [[lab-notes/2026-04-27-Music-Chords-Foundations-Anatomy-Harmony-and-Scale-Relat|Music Chords: Foundations, Anatomy, Harmony, and Scale Relationships]] · [▶ source](https://www.youtube.com/watch?v=Uyr-GogTrls)
