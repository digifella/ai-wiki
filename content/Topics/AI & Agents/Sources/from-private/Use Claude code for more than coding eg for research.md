...
Here is a [[concepts/summary|summary]] of the video titled **"How to Make [[concepts/claude-code|Claude Code]] Cook"**, detailing how to repurpose [[entities/anthropic|Anthropic]]'s [[concepts/coding|coding]] tool into a general-[[concepts/purpose|purpose]] [[concepts/autonomous-agent-system|autonomous agent system]].

# Hot Take: Claude Code isn't just for Coding

The core premise of the video is that **Claude Code is essentially an agentic [[concepts/loop|loop]]** wrapped in a coding-focused system message. By changing the objectives and [[concepts/instructions|instructions]] you provide, you can transform it from a coding assistant into a [[concepts/general-purpose-automation-tool|general-purpose automation tool]] capable of handling research, data analysis, [[concepts/content-creation|content creation]], and file management.

## The "Open Agent System" [[concepts/architecture|Architecture]]

The [[entities/speaker|speaker]] demonstrates a custom file-based architecture called the **Open Agent System**. Instead of [[concepts/writing|writing]] complex [[concepts/software|software]], this system relies on [[concepts/markdown|Markdown]] [[concepts/files|files]] to define "[[concepts/agents|agents]]."
...
3. **Agent Files**: Each agent is a Markdown file defining:
	...
	* **Tools:** What [[concepts/terminal-command-execution|shell commands]] or scripts it can run.
...
| **[[concepts/integration|Integration]]** | Native, elegant, highly shareable. | Manual [[concepts/setup|setup]] via Markdown files. |
| **Compatibility** | **Claude Code Only.** | **Universal.** Works with Claude Code, [[concepts/codex|Codex]], [[entities/cursor|Cursor]], [[concepts/gemini-cli|Gemini CLI]], etc. |
| **Context Usage** | Loads _all_ [[concepts/skill|skill]] definitions into context immediately (can be token heavy). | Uses **Progressive Disclosure** (loads instructions only when needed). |

The speaker [[concepts/notes|notes]] that while "Skills" are the future for Claude, the custom Markdown approach is currently more flexible across different [[concepts/ai-coding|AI coding]] tools.

* * *

## How to Try It (In < 3 Minutes)

You don't need to manually build the files to start. The speaker provided a [[entities/github|GitHub]] repository that Claude Code can read to bootstrap itself.
...
- [[concepts/open-agent-system|Open Agent System]] — [Wikipedia](https://en.wikipedia.org/wiki/Open_Agent_System)
- [[concepts/autonomous-ai-coding-agent|Autonomous Agent]] — [Wikipedia](https://en.wikipedia.org/wiki/Autonomous_Agent)
- [[concepts/coding-assistant|Coding Assistant]] — [Wikipedia](https://en.wikipedia.org/wiki/Coding_Assistant)

## Related Entities
- [[entities/claude-code-anthropic|Claude Code (Anthropic)]] — [Wikipedia](https://en.wikipedia.org/wiki/Claude_Code_%28Anthropic%29)
- [[concepts/open-agent-system-architecture|Open Agent System Architecture]] — [Wikipedia](https://en.wikipedia.org/wiki/Open_Agent_System_Architecture)