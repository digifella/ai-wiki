---
wiki-ingested: true
title: "Adam Lucek - Flux model for Open AI generated image gen"
...
This detailed [[concepts/summary|summary]] covers the key aspects of the video, including the goal, the [[concepts/ai-models|AI models]] and techniques used, the tools and [[concepts/setup|setup]] process, and the results.

* * *

**Video Title:** [[concepts/training|Training]] [[entities/flux1|FLUX.1]] ([[entities/black-forest-labs|Black Forest Labs]]) [[concepts/lora-adapter|LoRA Adapter]] on [[concepts/faces|Faces]] | Low VRAM Image Generation
**Main Goal:** To demonstrate how to train [[concepts/ai-image-generation|AI image generation]] [[concepts/models|models]] on faces to reliably recreate pictures of a specific person (the [[entities/speaker|speaker]], Adam Lucek, in this case) in various [[concepts/scenarios|scenarios]].
**Key Technologies & Concepts:**

1. **FLUX.1 by Black Forest Labs:**
	A 12 billion parameter [[concepts/rectilinear-flow-transformer|rectilinear flow transformer]] capable of generating [[concepts/high-quality-images|high-quality images]] from text descriptions. Considered a leading [[concepts/open-source|open-source]] [[concepts/image-generation-model|image generation model]]. Has generated significant [[concepts/community-interest|community interest]] and commercial applications (e.g., Photo AI .com).
	
2. **LoRA (Low-Rank Adaptation):**
	A parameter-efficient [[concepts/fine-tuning|fine-tuning]] method. Adds additional, small [[concepts/weights|weights]] (the LoRA adapter) on top of a pre-trained [[concepts/foundation-model|foundation model]] (FLUX.1). Only these small added weights are trained, making the process very efficient and requiring significantly less VRAM compared to full [[concepts/model-fine-tuning|model fine-tuning]]. The trained LoRA adapter is a small file that can be plugged in or out of the main FLUX.1 model during [[concepts/inference|inference]].
...
3. **[[concepts/running|Running]] the Training Script:**
...
1. **FLUX.1 (Black Forest Labs):** A 12-billion-parameter rectilinear flow transformer renowned for high-quality [[concepts/text-to-image-generation|text-to-image generation]]. It serves as the powerful base model for fine-tuning.
...
* **Overtraining:** The video notes that continued training can sometimes lead to a "degeneration" of general image quality (e.g., blurrier features) even as the facial resemblance improves. This means the LoRA adapter becomes too specialized and loses the base model's broader capabilities.
...
2. **Model Loading:** The workflow includes nodes to: Load the base FLUX.1 Diffusion Model. Load the DualCLIPLoader (for the text encoders). Load the VAE (Variational AutoEncoder). **Load LoRA Adapter:** This crucial node points to your saved `.safetensors` file.
