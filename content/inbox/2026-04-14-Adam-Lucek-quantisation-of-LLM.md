---
wiki-ingested: true
title: "Adam Lucek - quantisation of LLM"
...
---
# [[entities/adam-lucek|Adam Lucek]] - [[concepts/quantisation|quantisation]] of LLM

---
---
Adam Lucek - quantisation of LLM
<https://www.youtube.com/watch?v=3EDI4akymhA>
This video provides a detailed overview of quantization in the context of [[concepts/large-language-models|large language models (LLMs)]], explaining what it is, why it's necessary, and how it's implemented.
**1\. The Challenge of Large Language [[concepts/models|Models]]:** LLMs like [[entities/nvidia|NVIDIA]]'s Llama 3.1 [[entities/nemotron|Nemotron]] 70B (70.6 billion [[concepts/parameters|parameters]]) are massive, often requiring gigabytes of [[entities/storage|storage]] (e.g., 30+ [[concepts/files|files]], each ~5GB). [[concepts/running|Running]] these models at [[concepts/full-precision|full precision]] demands significant [[concepts/computational-resources|computational resources]], including expensive [[concepts/gpu-clusters|GPU clusters]] with large amounts of [[concepts/vram|VRAM]], making them inaccessible to most consumers.
...
* **In Deep Learning/ML:** It's a technique to reduce the computational and [[concepts/memory|memory]] costs of running [[concepts/inference|inference]] on models. This is achieved by representing the model's internal [[concepts/weights|weights]] and activations using **low-precision data types** (e.g., 8-bit integers or 4-bit integers) instead of the standard 32-bit [[concepts/floating-point-numbers|floating-point numbers]] (float32).
...
* **Inference [[concepts/speed|Speed]]:** Operations like matrix multiplication can be performed faster with integer arithmetic.
* **[[concepts/hardware|Hardware]] [[concepts/accessibility|Accessibility]]:** It allows models to run on less powerful, consumer-grade hardware (laptops, even CPUs) where previously only high-end GPUs or clusters were capable. Popular tools like [[entities/llama|OLLAMA]] and [[entities/lm-studio|LM Studio]] leverage quantization to run LLMs locally.
...
**Conclusion:** Quantization is a game-changer for LLM accessibility. By intelligently reducing the numerical precision of [[concepts/model-weights|model weights]], it significantly cuts down on memory and computational demands without a major drop in performance. This enables running powerful LLMs on a wide range of consumer hardware, fostering broader [[concepts/adoption|adoption]] and experimentation within the AI community.