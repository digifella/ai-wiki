...
Nacelle rotation is a mechanical system that allows the engines and rotors of a [[concepts/tiltrotor-technology|tiltrotor]] aircraft to change orientation during flight. In the [[concepts/vertical-takeoff-and-landing|V-22 Osprey]], each nacelle—the pod containing an engine and rotor assembly—can rotate from a vertical position (pointing upward for helicopter-like flight) to a horizontal position (pointing forward for fixed-wing flight). This rotation is essential to the aircraft's ability to transition between the two flight modes.
