...
This video features a discussion among four researchers from Anthropic about the nature of large language models (LLMs) and their work in "[[concepts/interpretability|interpretability]]."
**The Nature of Large Language Models (LLMs): More Than Auto-complete**
Stuart Ritchie, from Anthropic's Research Communications team, opens the discussion by posing fundamental questions: What exactly are we talking to when we interact with an LLM? Is it merely a "glorified auto-complete" or an "internet search engine," or is it "actually thinking, and maybe even thinking like a person"? He [[concepts/notes|notes]] that "nobody really knows the [[concepts/solution|answer]] to those questions," but Anthropic is deeply invested in finding out through interpretability research.
...
* **"Golden Gate Bridge"**: Joshua Batson explains that the model forms a "robust notion" of the Golden Gate Bridge that goes beyond just the words, akin to a mental image or the experience of driving across it.
...
* **"6 + 9"**: Jack Lindsey highlights a circuit that activates when the model needs to add numbers ending in 6 and 9, regardless of the full number (e.g., 36+59, or 6+9). This demonstrates that the model isn't just memorizing specific answers but learns a "general circuit" for addition.
* **Cross-Lingual Concepts**: Emmanuel points out that LLMs, capable of answering in multiple languages (like French or Japanese), often share representations for concepts (e.g., "big") across languages, rather than [[concepts/learning|learning]] 10 separate versions of each word. This indicates a deeper, universal conceptual understanding.
...
The ultimate goal is to understand the "causal mechanisms" – how the internal components lead to specific outputs. This allows for targeted improvements and helps address the "alien" nature of LLM thought processes. By "nudging" specific internal circuits and observing the results, researchers can test hypotheses about how the models work. This allows for a "trust basis" in AI, where humans can gradually understand and predict [[concepts/model-behavior|model behavior]].
...
## Related Concepts
- [[concepts/internet-search-engine|Internet search engine]] — [Wikipedia](https://en.wikipedia.org/wiki/Internet_search_engine)
- Emergent behavior — [Wikipedia](https://en.wikipedia.org/wiki/Emergent_behavior)
- [[concepts/interpretability|Interpretability]] research — [Wikipedia](https://en.wikipedia.org/wiki/Interpretability_research)
...
- Meta-objective — [Wikipedia](https://en.wikipedia.org/wiki/Meta-objective)
- Pattern matching — [Wikipedia](https://en.wikipedia.org/wiki/Pattern_matching)
- Robust notion — [Wikipedia](https://en.wikipedia.org/wiki/Robust_notion)
- [[concepts/conceptual-understanding|Universal conceptual understanding]] — [Wikipedia](https://en.wikipedia.org/wiki/Universal_conceptual_understanding)

## Related Entities
- [[entities/stuart-ritchie|Stuart Ritchie]] — [Wikipedia](https://en.wikipedia.org/wiki/Stuart_Ritchie)
- [[entities/anthropic|Anthropic]] — [Wikipedia](https://en.wikipedia.org/wiki/Anthropic)
- Internet search engine — [Wikipedia](https://en.wikipedia.org/wiki/Internet_search_engine)