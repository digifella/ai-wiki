...
Here is a [[concepts/markdown|Markdown]] [[concepts/summary|summary]] and guide based on the video content.

# Running [[concepts/claude-code|Claude Code]] Locally with [[entities/llama|Ollama]] and GLM-4.7-Flash
...
Ollama now supports the Anthropic API, allowing users to hook local [[concepts/models|models]] into tools designed for Claude. This demonstration tests the **GLM-4.7-Flash** model (a 30B parameter [[entities/mixture-of-experts|Mixture-of-Experts]] model with 3B active [[concepts/parameters|parameters]]) to see if it can function as a [[concepts/coding|local coding]] assistant.
...
* **[[concepts/hardware|Hardware]]:** Recommended to have a Mac with [[entities/apple|Apple]] Silicon (M-series) or a machine with a powerful GPU. Tested on Mac Mini Pro (32GB [[concepts/ram|RAM]]).

## Step-by-Step [[concepts/setup|Setup]]

### 1\. Pull the Model

Open your terminal and pull the [[entities/glm|GLM]] model:
```
...
By default, Ollama uses a context length of **4096 tokens**. This is insufficient for coding [[concepts/agents|agents]] like Claude Code, which [[entities/will|will]] cause the model to forget instructions or fail to use tools.
...
* **[[concepts/cost|Cost]]:** It allows for a free "backup" to the paid Anthropic API.

### ❌ The Bad

* **[[concepts/speed|Speed]]:** It is significantly slower than the hosted Claude API. Both "pre-fill" (processing context) and "decoding" (generating text) take a long time on local hardware.
...
* Future optimized coding models (like [[concepts/gemini|Gemini]] 4 or [[entities/qwen|Qwen]] 4) may make this viable soon.

## Related Concepts
- [[concepts/local-model|local model]] — [Wikipedia](https://en.wikipedia.org/wiki/local_model)
- [[concepts/coding-assistant|coding assistant]] — [Wikipedia](https://en.wikipedia.org/wiki/coding_assistant)
- [[concepts/ollama-launch-command|Anthropic API]] — [Wikipedia](https://en.wikipedia.org/wiki/Anthropic_API)
...
- [[entities/glm-47-flash|GLM-4.7-Flash]] — [Wikipedia](https://en.wikipedia.org/wiki/GLM-4.7-Flash)