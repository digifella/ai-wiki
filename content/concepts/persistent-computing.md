...
Persistent computing refers to a technique for maintaining long-[[concepts/running|running]] processes and sessions that continue operating independently of the terminal or [[concepts/connection|connection]] through which they were initiated. The method relies on [[concepts/session-management|session management]] tools, most commonly tmux ([[entities/tmux|terminal multiplexer]]), which allows users to [[concepts/detach|detach]] from an active [[concepts/session|session]] and reattach to it later without interrupting the processes running within that session.
