...
# Ollama + [[concepts/claude|Claude]] + GLM. Channel [[entities/sam-witteveen|Sam Witteveen]]
...
# [[concepts/running|Running]] [[concepts/claude-code|Claude Code]] Locally with Ollama and [[entities/glm-47-flash|GLM-4.7-Flash]]
...
Ollama now supports the Anthropic API, allowing users to hook local models into tools designed for Claude. This demonstration tests the **GLM-4.7-Flash** model (a 30B parameter [[concepts/mixture-of-experts|Mixture-of-Experts]] model with 3B [[concepts/active-parameters|active parameters]]) to see if it can function as a local [[concepts/coding-assistant|coding assistant]].
...
* **[[concepts/hardware|Hardware]]:** Recommended to have a Mac with Apple [[concepts/silicon|Silicon]] (M-series) or a machine with a powerful GPU. Tested on Mac Mini Pro (32GB RAM).
...
By default, Ollama uses a context length of **4096 tokens**. This is insufficient for coding [[concepts/agents|agents]] like Claude Code, which will cause the model to forget [[concepts/instructions|instructions]] or fail to use tools.
...
Run the following command in your terminal to initialize the [[concepts/connection|connection]]:
```
...
Once launched, you can interact with Claude Code using the [[concepts/local-model|local model]] just as you would with the hosted version (e.g., using `/plan` mode).
...
While `ollama launch` is an excellent feature for ease of use, running Claude Code locally with current [[concepts/open-weights-models|open-weights models]] on consumer hardware is **not yet "prime time" ready** for professional workflows.
