...
Context
Paul needs a way to create and edit travel blogs visually — similar to [[entities/adobe-express|Adobe Express]] — with sequential blocks of text and photos. The existing site/journeys/ system stores blogs as static HTML in content/{slug}.html, but there's no editor. The homepage nav's second row (quick-nav) is being obscured by the ticker bar. Two Japan trip PDFs need to be parsed and fused into one blog post.
...
* Safe error responses (generic error + request\_id, details to server log) — we'll follow same pattern
* Deploy script now uses git ls-[[concepts/files|files]] — new files must be committed before ./deploy.sh includes them
* Session hardening on auth.php — already in place, no extra work needed
...
7. Parse Japan PDFs → HTML + images via [[entities/python|Python]] script
...
[[entities/claude-4|Claude]] has written up a plan and is ready to execute. Would you like to proceed?
...
## Related Concepts
- [[concepts/sequential-editing|sequential editing]] — [Wikipedia](https://en.wikipedia.org/wiki/sequential_editing)
- [[concepts/visual-blog-creation|visual blog creation]] — [Wikipedia](https://en.wikipedia.org/wiki/visual_blog_creation)
- [[concepts/journey-management|journey management]] — [Wikipedia](https://en.wikipedia.org/wiki/journey_management)
- [[concepts/ticker-bar-navigation|ticker bar navigation]] — [Wikipedia](https://en.wikipedia.org/wiki/ticker_bar_navigation)
