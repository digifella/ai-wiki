...
Source: [[entities/langchain|Langchain]]

This video showcases a "[[entities/google-gemini-25|Gemini 2.5]] Multi-modal researcher," a tool built using [[entities/gemini-ai|Google's Gemini]] 2.5 models through [[concepts/langgraph-workflows|LangGraph]]. The researcher is designed to perform comprehensive investigations and generate various outputs, leveraging Gemini's native capabilities.
**Core Functionality & Workflow:** The researcher operates by taking two main inputs:
...
3. **Create Report:** Synthesizes the information gathered from both web search and video analysis into a comprehensive research report, including an executive summary and cited sources.
4. **Create Podcast:** Generates an engaging, multi-[[entities/speaker|speaker]] podcast script based on the synthesized research. It then uses Gemini 2.5's [[concepts/text-to-speech-synthesis|text-to-speech]] (TTS) model with distinct voices (e.g., "Mike" and "Dr. Sarah") to produce an audio file of the podcast.
...
* **Large [[concepts/context-window|Context Window]]:** Gemini 2.5 supports up to 1 million tokens, enabling the processing of extensive amounts of information.
...
**Demonstration & [[concepts/implementation-details|Implementation Details]]:** The video shows a live demo where a topic and a YouTube URL are input, and the system progresses through the research, analysis, reporting, and podcast creation stages. The LangSmith trace is used to visualize the execution flow and inspect the inputs and outputs of each internal LLM call, including the extracted video text and the full research report. The generated podcast is played, demonstrating the quality of the multi-speaker audio.
...
**Overall Impression:** The speaker expresses strong positive feedback on the Gemini 2.5 models, noting their impressive performance on various benchmarks (Text, WebDev, [[concepts/computer-vision|Vision]], Search) and highlighting the groundbreaking nature of the multi-speaker text-to-speech feature for creating dynamic audio content. The multi-modal researcher serves as a practical and accessible example of how these new capabilities can be harnessed for automated research and content generation.
...
## Related Concepts
- [[concepts/multi-modal-researcher|Multi-modal researcher]] — [Wikipedia](https://en.wikipedia.org/wiki/Multi-modal_researcher)
- [[concepts/gemini-25-models|Gemini 2.5 models]] — [Wikipedia](https://en.wikipedia.org/wiki/Gemini_2.5_models)
- [[concepts/native-capabilities|native capabilities]] — [Wikipedia](https://en.wikipedia.org/wiki/native_capabilities)
- [[concepts/comprehensive-investigations|comprehensive investigations]] — [Wikipedia](https://en.wikipedia.org/wiki/comprehensive_investigations)
- [[concepts/various-outputs|various outputs]] — [Wikipedia](https://en.wikipedia.org/wiki/various_outputs)

## Related Entities
- organization — [Wikipedia](https://en.wikipedia.org/wiki/organization)
- person — [Wikipedia](https://en.wikipedia.org/wiki/person)
- product — [Wikipedia](https://en.wikipedia.org/wiki/product)