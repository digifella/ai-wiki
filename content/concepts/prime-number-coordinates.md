...
When primes are plotted using various coordinate schemes—such as using consecutive primes as (x, y) pairs or mapping primes to positions on a grid—distinct visual patterns emerge. These patterns often reveal unexpected gaps, clusters, and alignments that highlight the irregular [[concepts/prime-number-distribution|distribution of primes]]. The concept gained popular [[concepts/attention-mechanisms|attention]] through [[entities/numberphile|Numberphile]]'s "[[concepts/awkward-primes|Awkward Primes]]" series, which examines primes that create particularly unusual or sparse geometric configurations.
