...
Here is a [[concepts/markdown|Markdown]] [[concepts/summary|summary]] and step-by-step guide based on the video [[concepts/text-transcript|transcript]].

# How to Run Claude Code Locally for Free (Using [[entities/ollama|Ollama]])

**Claude Code** is a powerful agentic [[concepts/coding|coding]] tool that can build applications, fix errors, and refactor code. However, the official API (e.g., [[entities/opus|Opus]] 4.6) can be expensive ($5 input / $25 output per million [[concepts/tokens|tokens]]).
...
* A computer capable of [[concepts/running|running]] local LLMs.
* Terminal access.
...
* **Interaction:** You will be asked to confirm edits. Press `Enter` (Yes) or `Shift+Tab` (Allow all edits in [[concepts/session|session]]) to proceed.
