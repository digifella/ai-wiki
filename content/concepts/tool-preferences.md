---
type: concept
domain: tools-platforms
tags:
  - "tool preferences"
  - "photoshop"
  - "brush tool"
  - "adobe-photoshop"
  - "brush-tool-shortcuts"
  - "digital-art-tools"
  - "design-software-preferences"
aliases:
  - "Photoshop Brush Shortcuts"
  - "Brush Tool Settings"
summary: "This page details keyboard shortcuts for adjusting brush size, hardness, and the heads-up display within Adobe Photoshop."
updated: 2026-04-17
group: developer-tooling-clis
---
# Tool Preferences

Preferences for tools in digital art and [[concepts/design|design]] [[concepts/software|software]], particularly focusing on the Brush Tool in [[entities/adobe-photoshop]].

## Adobe Photoshop Brush Tool Shortcuts
* **Bracket Keys:** Tap `[` or `]` to decrease or increase [[concepts/brush-size|brush size]].
* **Shift + Bracket Keys:** Hold **Shift** and tap `[` or `]` to change edge hardness.
* **[[concepts/hud-heads-up-display|Heads Up Display]] ([[concepts/hud|HUD]]):**
  - **Mac:** Hold **Control + Option** and drag.
  - **[[entities/windows|Windows]]:** Hold **Control + Alt + Right-click** and drag.

2026 04 14 [[entities/julianne-kost|Julianne Kost]] using brush tool in photoshop

## Source Notes
- 2026-04-23: GPT 5 · [▶ source](https://www.youtube.com/watch?v=xbvI5G-8q4o)
- 2026-04-07: [[lab-notes/2026-04-07-Claude-AI-Excel-Add-in-for-Financial-Modeling-Overview-and-Tutorial|Claude AI Excel Add in for Financial Modeling Overview and Tutorial]] · [▶ source](https://www.youtube.com/watch?v=iEh53QLluNw)
- 2026-04-08: [[lab-notes/2026-04-08-Claude-Cowork-Desktop-AI-Co-worker-Core-Capabilities-and-Advantages|Claude Cowork Desktop AI Co worker Core Capabilities and Advantages]] · [▶ source](https://www.youtube.com/watch?v=z9rdrNrkvDY)
- 2026-04-10: [[lab-notes/2026-04-10-Google-NotebookLMs-Latest-Features-Enhanced-Infographics-AI-Videos|Google NotebookLMs Latest Features Enhanced Infographics AI Videos]] · [▶ source](https://www.youtube.com/watch?v=E71M74FIDHc)
- 2026-04-15: [[lab-notes/2026-04-15-Hermes-Agent-Self-Improving-AI-for-Adaptive-User-Learning|Hermes Agent Self Improving AI for Adaptive User Learning]] · [▶ source](https://www.youtube.com/watch?v=5PLDovsqKaQ)
- 2026-04-22: Lightroom Classic · [▶ source](https://youtu.be/K70wThvpHFM)