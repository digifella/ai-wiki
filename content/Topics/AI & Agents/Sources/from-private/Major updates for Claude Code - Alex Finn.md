...
Resume old sessions. When you go in a project and run this command, you can see all your old [[concepts/claude-code|Claude Code]] sessions and resume whichever one you want so you don’t lose context: claude —resume
See what takes up your context. Running this command shows you what takes up your [[concepts/context-window|context window]]. Great to use when Claude is hallucinating a lot or not cooperating: /context
...
Based on the video, here are the 10 secret [[concepts/instant-auto-compaction|Claude Code updates]] that can significantly improve your [[concepts/workflow|workflow]]:
...
3. **Usage Statistics (**`**/stats**`**)**: Use the `/stats` command to track your activity. It provides a dashboard showing your token usage, favorite [[concepts/models|models]] (like [[entities/opus|Opus]] or Sonnet), current streaks, and peak usage hours, helping you manage your plan limits.
4. **Name Your Sessions (**`**/rename**`**)**: To keep your resumed sessions organized, use the `/rename` command followed by a custom name (e.g., `/rename kanban-board`). This makes it much easier to identify specific tasks when browsing through your session history.
...
7. **Custom Memories (**`**#**`**)**: You can now manually add information to Claude’s long-term [[concepts/memory|memory]] without editing the `CLAUDE.md` file directly. Simply start a message with the **hashtag symbol (**`**#**`**)** to enter memory mode and save project-specific instructions (e.g., "# Always use the design skill for UI changes").
8. **YOLO Mode**: For [[concepts/power-users|advanced users]] who want Claude to work autonomously, use the flag `claude --dangerously-skip-permissions`. This prevents Claude from stopping to ask for permission for every individual command or file edit, allowing it to run uninterrupted.
9. **Plugin Store (**`**/plugins**`**)**: The `/plugins` command acts like an app store for Claude Code. It allows you to discover and install "[[concepts/skills|skills]]" and [[concepts/model-context-protocol|Model Context Protocol (MCP)]] integrations—such as specialized front-end design tools or Notion integrations—with a single click.
...
## Related Concepts
- [[concepts/contextual-window|contextual window]] — [Wikipedia](https://en.wikipedia.org/wiki/contextual_window)
- [[concepts/hallucination-rate|hallucination]] — [Wikipedia](https://en.wikipedia.org/wiki/hallucination)
- [[concepts/usage-statistics|usage tracking]] — [Wikipedia](https://en.wikipedia.org/wiki/usage_tracking)
- [[concepts/code-rewriting|code rewriting]] — [Wikipedia](https://en.wikipedia.org/wiki/code_rewriting)

## Related Entities
- None (no named author/channel) — [Wikipedia](https://en.wikipedia.org/wiki/None_%28no_named_author/channel%29)