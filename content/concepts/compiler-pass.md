...
A **compiler pass** is a discrete stage in a compiler pipeline that performs either analysis or transformation on an Intermediate Representation ([[concepts/incident-response|IR]]).

### Core Classifications
- **Analysis Passes**: Inspect the program [[concepts/structure|structure]] or properties (e.g., [[concepts/power|Control]] [[concepts/flow|Flow]] Graph analysis, Data Flow Analysis) without modifying the [[concepts/code|code]].
- **Transformation Passes**: Modify the IR to optimize for performance, size, or power (e.g., Dead [[concepts/software|Code]] Elimination, [[concepts/loop|Loop]] Unrolling, Instruction Scheduling).
