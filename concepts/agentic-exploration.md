...
Agentic exploration refers to the capability of [[concepts/agentic-ai|AI agents]] to autonomously navigate and retrieve information from [[concepts/external-knowledge|external knowledge]] sources in [[entities/pursuitunimelbeduau|pursuit]] of task objectives. Unlike traditional [[concepts/answer-generation|retrieval-augmented generation]] (RAG), which typically performs a single [[concepts/document-retrieval|retrieval]] pass based on an initial query, agentic exploration enables systems to make deliberate decisions about when and how to search, what queries to formulate, and how to iteratively synthesize retrieved information. This approach treats [[concepts/knowledge-bases|information retrieval]] as an active [[concepts/reasoning-steps|reasoning process]] rather than a passive lookup step.

## Core Mechanisms

[[concepts/agentic-frameworks|Agentic systems]] assess the sufficiency of available information at each step and determine whether additional retrievals are necessary to complete a task. The agent can reformulate queries based on intermediate results, explore related topics, and decide when enough evidence has been gathered. This requires integration of planning capabilities, confidence evaluation, and control [[concepts/flow|flow]] [[concepts/open-source-philosophy|logic]] that allows the agent to direct its own search strategy across multiple iterations.

## Relationship to Prompt Engineering

Agentic exploration is closely related to advances in [[concepts/prompt-based-modeling|prompt engineering]], particularly techniques that encourage [[concepts/multi-step-reasoning|step-by-step reasoning]] and explicit planning. Clear [[concepts/instructions|instructions]] about when to search, how to evaluate retrieved content, and when to stop [[concepts/retrieving|retrieving]] improve agent performance. Effective prompts define the agent's [[concepts/decision-making|decision-making]] criteria and help establish appropriate stopping conditions, preventing both premature conclusion and excessive redundant searches.
