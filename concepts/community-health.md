---
type: concept
domain: health-wellbeing
tags:
  - "community-health"
  - "nursing"
  - "public-health"
  - "preventive-care"
  - "health-promotion"
  - "population-health"
  - "patient-education"
  - "case-management"
aliases:
  - "Community Health Nursing"
  - "CHN"
  - "Population Health Care"
  - "Community Public Health"
summary: Community health nursing involves the promotion and maintenance of population health through direct care, education, and policy implementation.
updated: 2026-07-11
group: health-practice-patient-knowledge
generated: { by: "process:nemoclaw-wiki-ingest", at: "2026-07-11" }
---
<!-- domain-nav -->
> domain-badge slug=health-wellbeing name=Health & Wellbeing

# Community Health Nursing Overview

[[concepts/community-health-nursing|Community health nursing]] focuses on promoting and maintaining the [[concepts/health|health]] of populations within communities through various interventions, education [[concepts/software|programs]], and [[concepts/preventive-care|preventive care]] strategies. It encompasses a broad spectrum of activities designed to address [[concepts/public-health|public health]] issues at local levels, emphasizing [[concepts/primary-prevention|primary prevention]] and [[concepts/health-promotion|health promotion]].

## Core Concepts in Community Health Nursing
- Population-focused [[concepts/health-care|healthcare]]
- [[concepts/population-health|Public health]] [[concepts/policies|policies]] and regulations
- Environmental factors affecting community health

### Key Roles in Community Health Nursing
- **Community Health Nurse (CHN):** Provides direct care, conducts health assessments, educates the public on health-related issues, and collaborates with other professionals to address community needs.
- **Case Manager:** Coordinating comprehensive care for individuals or families within a specific geographic area who have complex medical conditions or social challenges.
- **Public Health [[concepts/phil-moriarty|Educator]]:** Developing educational materials and programs aimed at increasing [[concepts/conscious-thought|awareness]] about healthy lifestyles and [[concepts/health-promotion|disease prevention]].

### Related Concepts
- [[concepts/health-promotion|Health Promotion]]
- [[concepts/preventive-care|Preventive Care]]
- [[concepts/community-health-programs|Community Health Programs]]

## New Note Integration (2026-04-11)
- The video by [[entities/ellis-parker|Ellis Parker]] from [[entities/level-up-rn|Level Up RN]] offers a comprehensive look into the fundamental [[concepts/terminology|terminology]] and roles in [[concepts/fundamentals-of-nursing|community health nursing]].
  - **[[concepts/terminology|Terminology:]]**
    - **Primary Prevention:** Interventions designed to prevent disease before it occurs.
    - **[[concepts/secondary-prevention|Secondary Prevention]]:** [[concepts/secondary-prevention|Early detection and treatment]] of diseases to limit their progression.
    - **[[concepts/tertiary-prevention|Tertiary Prevention]]:** Measures aimed at preventing further deterioration for those already diagnosed with chronic conditions.
  - **[[concepts/roles-in-community-settings|Roles in Community Settings:]]**
    - [[concepts/conducting-health-screenings|Conducting health screenings]]
    - [[concepts/providing-vaccinations|Providing vaccinations]]
    - [[concepts/offering-counseling-services|Offering counseling services]]

### Backlinks
- 2026 04 11 [[concepts/community-health-nursing|Community Health Nursing]] Core Terminology and [[concepts/nursing-roles|Nursing Roles]]
## Source Notes
- 2026-04-11: how climate affects community health - full video
