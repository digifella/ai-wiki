---
type: concept
domain: tools-platforms-infrastructure
tags:
  - "photography-workflow"
  - "file-import"
  - "lightroom-classic"
  - "assisted-culling"
  - "auto-stacking"
  - "digital-asset-management"
  - "cloud-integration"
aliases:
  - "Photo Import"
  - "Ingest Process"
  - "Library Integration"
summary: The import process involves transferring photographs and multimedia from various sources into editing or cataloging software, utilizing features like AI-powered culling, auto-stacking, and cloud integration for efficient
updated: 2026-07-11
group: automation-scheduling-sync
generated: { by: "process:nemoclaw-wiki-ingest", at: "2026-07-11" }
---
<!-- domain-nav -->
> domain-badge slug=tools-platforms-infrastructure name=Tools, Platforms & Infrastructure

# Import Process

The **Import Process** in digital [[concepts/photography-workflow|photography workflow]] management refers to how photographs and multimedia files are brought into a [[concepts/photo-editing|photo editing]] or cataloging software. It encompasses the initial steps taken when integrating new content from various sources such as cameras, [[concepts/memory|memory]] cards, and cloud [[entities/storage|storage]] into a user's library.

- Utilizes advanced features like AI-powered culling and auto-stacking for enhanced [[concepts/productivity|productivity]].
- Supports batch importing with customizable [[concepts/metadata|metadata]] tags for efficient categorization.
- Integrates seamlessly with [[concepts/cloud-based-solutions|cloud services]] for [[concepts/remote-access|remote access]] and collaboration.

### New Features in Lightroom Classic

- **[[concepts/assisted-culling|Assisted Culling]] (Early Access)** - Automatically evaluates photos based on quality, [[concepts/writing|composition]], and other criteria to help users quickly identify their best shots. AI-Powered [[concepts/assisted-culling|Assisted Culling]]
- **Auto Stacking** - Automatically groups related images such as burst captures or bracketed exposures into stacks for easier management and editing.

### Related Concepts
- [[concepts/metadata|Metadata]] Tagging
- [[concepts/batch-import|Batch Import]]
- [[concepts/cloud-integration|Cloud Integration]]

## References

* [Lightroom Classic Early Access: AI-Powered Assisted Culling and Auto Stacking](https://www.youtube.com/watch?v=F5yy-XpLXOs)
  *Clip title:* [[concepts/assisted-culling|Assisted Culling]] (Early Access) & Auto Stacking in [[concepts/lightroom|Lightroom]] Classic
  *[[entities/tasia-custode|Author]] / channel:* [[entities/julieanne-kost|Julieanne Kost]]

2026 04 13 [[entities/lightroom-classic|Lightroom Classic]] Early Access AI Powered Assisted Culling and Auto St
## Source Notes
- 2026-04-07: [[lab-notes/2026-04-07-Google-Gemini-and-NotebookLM-Key-Updates-and-Enhanced-AI-Integration|Google Gemini and NotebookLM Key Updates and Enhanced AI Integration]] · [▶ source](https://www.youtube.com/watch?v=6YWPGjqOEmk)
- 2026-04-13: [[lab-notes/2026-04-13-Lightroom-Classic-Early-Access-AI-Powered-Assisted-Culling-and-Auto-St|Lightroom Classic Early Access AI Powered Assisted Culling and Auto St]] · [▶ source](https://www.youtube.com/watch?v=F5yy-XpLXOs)
- 2026-04-21: [[lab-notes/2026-04-21-Automating-Travel-Itineraries-with-AI-and-Google-My-Maps|Automating Travel Itineraries with AI and Google My Maps]] · [▶ source](https://www.youtube.com/watch?v=fI8ygOauNNA)
- 2026-04-22: Lightroom Classic · [▶ source](https://youtu.be/K70wThvpHFM)
- 2026-04-23: Excel
