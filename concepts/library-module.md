---
type: concept
domain: tools-platforms-infrastructure
tags:
  - "adobe-lightroom"
  - "photo-management"
  - "library-module"
  - "image-organization"
aliases:
  - "Lightroom Library"
  - "LR Classic Library"
summary: The Library Module is a component of Adobe Lightroom Classic used to import, organize, tag, and rate digital photographs.
updated: 2026-07-11
group: developer-tooling-clis
generated: { by: "process:nemoclaw-wiki-ingest", at: "2026-07-11" }
---
<!-- domain-nav -->
> domain-badge slug=tools-platforms-infrastructure name=Tools, Platforms & Infrastructure

## Library Module
The **Library Module** is a key component of [[entities/adobe-lightroom|Adobe Lightroom]] Classic, designed to manage and organize digital photographs efficiently. It provides tools for importing photos from various sources, tagging and rating images, creating collections, and more.

### Key Features
- **Import**: Streamlines the process of bringing photos into Lightroom.
- **Collections**: Organizes photos into groups for easy access and management.
- **[[concepts/keywords|Keywords]] & Tags**: Adds [[concepts/metadata|metadata]] to help locate images quickly.
- **Ratings**: Uses [[concepts/star-ratings|star ratings]] to prioritize and sort photos.
- **Develop Previews**: Creates previews for faster workflow.

### New Early Access Features
- **[[concepts/assisted-culling|Assisted Culling]] (Early Access)**: Utilizes AI technology to suggest which photos should be kept or discarded, enhancing the efficiency of [[concepts/photo-culling|photo selection]].
- **Auto Stacking**: Automatically groups similar images into stacks based on criteria like date and location, making it easier to manage duplicates.

### Related Concepts
- [[concepts/lightroom]]
- [[entities/adobe|Adobe]] [[entities/creative-cloud|Creative Cloud]]

### Backlinks
2026 04 13 [[entities/lightroom-classic|Lightroom Classic]] Early Access AI Powered [[concepts/assisted-culling|Assisted Culling]] and Auto St
## Source Notes
- 2026-04-13: Assisted Culling (Early Access) & Auto Stacking in Lightroom Classic
