...
Libdvdcss is a [[concepts/software|software]] library that provides decryption capabilities for CSS-protected DVDs. It enables media players and video conversion tools to read and process DVD content by handling the Content Scramble System (CSS) encryption that manufacturers apply to commercial DVDs.

## Usage

The library is commonly integrated into popular media applications including [[entities/vlc|VLC media player]], Handbrake, and [[entities/makemkv|MakeMKV]]. Users typically install libdvdcss alongside these tools to enable DVD playback and ripping functionality. VLC can open [[concepts/dvd-iso|DVD ISO]] files created from protected discs, though the files must be opened via drag-and-drop rather than through the standard menu options.
