---
type: concept
domain: creative-pursuits
tags:
  - "concept"
  - "lightroom-classic"
  - "presence-sliders"
  - "texture"
  - "clarity"
  - "dehaze"
  - "color-adjustment"
aliases:
  - "Texture Clarity and Dehaze"
  - "Lightroom Presence Controls"
summary: Lightroom Classic sliders for adjusting texture, clarity, and dehaze to enhance image presence and detail.
updated: 2026-07-12
group: lightroom-color-workflows
generated: { by: "process:nemoclaw-wiki-ingest", at: "2026-07-12" }
---
<!-- domain-nav -->
> domain-badge slug=creative-pursuits name=Creative Pursuits

# Presence Sliders

Presence sliders are a set of adjustment tools in [[entities/lightroom-classic|Lightroom Classic]] designed to enhance the perceived depth, detail, and visual impact of an image. Located in the Basic panel of the [[concepts/adobe-lightroom-develop-module|Develop module]], these sliders work by adjusting local [[concepts/contrast|contrast]] and [[concepts/texture|texture]] at different [[concepts/musical-scales|scales]], allowing photographers to add dimension and definition to their photos. The three primary presence sliders are Texture, [[concepts/clarity-slider|Clarity]], and Dehaze, each affecting the image in distinct ways.

## Texture

The [[concepts/texture-slider|Texture slider]] adjusts mid-[[concepts/tone|tone]] contrast at a fine scale, emphasizing surface details and micro-textures within an image. Positive values enhance tactile qualities, making fabrics, skin, and surfaces appear more defined, while negative values soften and smooth these details. This slider is particularly useful for bringing out surface characteristics without affecting the overall tonal structure.

## Clarity

Clarity operates on a broader scale than Texture, affecting [[concepts/image-contrast|local contrast]] across mid-tone areas. Increasing clarity adds punch and definition to an image by creating subtle halos around edges and increasing contrast between adjacent tones. This creates a perception of greater [[concepts/sharpness|sharpness]] and dimensionality, though excessive clarity can appear unnatural or overly processed.

## Dehaze

Dehaze reduces atmospheric haze and increases local contrast, particularly effective for images shot in hazy conditions or those lacking punch. Positive values cut through overcast [[concepts/light|light]] and atmospheric interference, while negative values can add a soft, atmospheric quality. This slider is especially valuable for landscape and outdoor photography where atmospheric conditions affect clarity.
## Source Notes
- 2026-04-22: Lightroom Classic · [▶ source](https://youtu.be/vMqQBPffFbQ)
- 2026-04-09: Photoshop
- 2026-04-10: [[lab-notes/2026-04-10-Photoshops-Blend-If-Pixel-Perfect-Transparency-via-Brightness-and-Colo|Photoshops Blend If Pixel Perfect Transparency via Brightness and Colo]] · [▶ source](https://www.youtube.com/watch?v=Wkti_IX3Qzk)
